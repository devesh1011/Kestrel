...
function LendContent() {
  ...
  useEffect(() => {
    ...
    async function fetchYield() {
      ...
      try {
        ...
        // Silent — chart stays empty, no crash
      } finally {
  ...
  const depositAmountParsed = depositAmt
    ? (() => { try { return parseUnits(depositAmt, 18); } catch { return 0n; } })()
    : 0n;
  const needsApproval = depositAmountParsed > 0n && allowance < depositAmountParsed;
  const depositDisabled = !depositAmt || !address || depositConfirming || approveConfirming;

  const withdrawAmountParsed = withdrawAmt
    ? (() => { try { return parseUnits(withdrawAmt, 18); } catch { return 0n; } })()
    : 0n;
  const withdrawDisabled =
    !withdrawAmt || !address || withdrawConfirming || withdrawAmountParsed > availableLiquidity;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card className="border-white/[0.06] bg-white/[0.04]">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-zinc-400">Your Position</CardTitle>
          </CardHeader>
          <CardContent>
            {!address ? (
        ...
        <Card className="border-white/[0.06] bg-white/[0.04]">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-zinc-400">Vault TVL</CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              <Skeleton className="h-7 w-28 bg-white/10" />
              ...
              <>
                ...
                <p className="mt-1 text-xs text-zinc-500">
                  {parseFloat(formatUnits(availableLiquidity, 18)).toFixed(2)} CTC available
                </p>
              </>
            )}
          </CardContent>
        ...
        <Card className="border-white/[0.06] bg-white/[0.04]">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-zinc-400">All-Time Interest</CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              <Skeleton className="h-7 w-28 bg-white/10" />
              ...
              <p className="text-2xl font-bold text-emerald-400">
                {parseFloat(formatUnits(totalInterestEarned, 18)).toFixed(4)} CTC
              </p>
            )}
          </CardContent>
        </Card>
          ...
          <Tabs defaultValue="deposit">
            ...
            <TabsContent value="deposit" className="space-y-4">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {depositHash && (
                  <TxStatusBadge
                    status={depositConfirming ? "pending" : depositConfirmed ? "confirmed" : "pending"}
                    txHash={depositHash}
                  />
                )}
              </div>
            ...
            <TabsContent value="withdraw" className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                />
                <Button
                  onClick={() => setWithdrawAmt(formatUnits(availableLiquidity, 18))}
                  variant="outline"
                  size="sm"
                  className="shrink-0 border-white/[0.10] bg-white/[0.05] text-zinc-300 hover:text-white"
                >
              ...
              {withdrawHash && (
                <TxStatusBadge
                  status={withdrawConfirming ? "pending" : withdrawConfirmed ? "confirmed" : "pending"}
                  txHash={withdrawHash}
                />
              )}
            </TabsContent>
        ...
        <CardContent>
          {yieldLoading ? (
            ...
            <div className="flex h-48 items-center justify-center text-sm text-zinc-500">
              No repayments yet — yield will appear here once loans are repaid.
            </div>
          ) : (
      ...
      <Card className="border-white/[0.06] bg-white/[0.04]">
        <CardHeader>
          <CardTitle className="text-sm font-medium text-white">Wrap CTC → WCTC</CardTitle>
          <CardDescription className="text-zinc-400 text-xs">
            The vault uses WCTC (ERC-20 wrapped CTC). Wrap native CTC here before depositing.
          </CardDescription>
        </CardHeader>
        <CardContent>
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-3">
        ...
        <Button
          ...
          variant="outline"
          className="shrink-0 border-blue-700 text-blue-400 hover:bg-blue-950/50 disabled:opacity-50"
        >
